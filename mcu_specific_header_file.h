...
typedef struct			// rest we will create when we will need them
{
		__IO	uint32_t CR1;
		__IO	uint32_t CR2;
		__IO	uint32_t OAR1;
...
// for enabling/desabling clock we need RCC section, so we need to have access to base addresse of RCC registers,
...
4. Addresses of Register (for that use structure)
5. Peripheral Definition (ie pointer type casting of base addess of a peripheral to pointer of struct typedef)
